...
//		motorVal = (int)((((200 - middleServoVal+OFS) - (servoVal - middleServoVal+OFS))/(float)(200 - middleServoVal+OFS))*maxMotorVal);
	...
	if (fc4st == 1 && fc5st == 1 && fc6st == 1 && fc7st == 1 && fc8st == 1 && fc9st == 1)
	{
...
//	//GPIO INITIALIZE FOR CONTROLS
	...
	cam.writeReg(0x350B, 0x75);  // this manually sets the gain
	...
	Xil_Out32(VIDEO_PROC_ADDR, 185); // turn white threshold to 200
	...
	Xil_Out32(VIDEO_PROC_ADDR+(4*61), 7); //lanecountval -> 5
